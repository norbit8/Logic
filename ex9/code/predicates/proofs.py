...
class Schema:
    ...
    def _instantiate_helper(formula: Formula,
        ...
        if is_relation(formula.root) and formula.root not in relations_instantiation_map or is_equality(formula.root):
            args = [t.substitute(constants_and_variables_instantiation_map, set()) for t in formula.arguments]
        ...
        if is_relation(formula.root) and formula.root in relations_instantiation_map:
            if len(formula.arguments) == 0:
                for var1 in relations_instantiation_map[formula.root].free_variables():
                ...
                t = formula.arguments[0].substitute(constants_and_variables_instantiation_map, set())
                for var1 in relations_instantiation_map[formula.root].free_variables():
                    if var1 in bound_variables:
                        raise Schema.BoundVariableError(var1, formula.root)
                try:
        ...
        if is_quantifier(formula.root):
            new_bound_variable = set(bound_variables)
            new_bound_variable.add(formula.variable)
            new_variable = formula.variable
            if formula.variable in constants_and_variables_instantiation_map:
                new_bound_variable.discard(formula.variable)
                new_variable = constants_and_variables_instantiation_map[formula.variable]
                new_bound_variable.add(str(new_variable))
            return Formula(formula.root, str(new_variable),
                           Schema._instantiate_helper(
                               formula.predicate,constants_and_variables_instantiation_map,
        ...
        if is_binary(formula.root):
            return Formula(formula.root, Schema._instantiate_helper(formula.first,
                                                                constants_and_variables_instantiation_map,
                                                                relations_instantiation_map,
                                                                bound_variables), Schema._instantiate_helper(formula.second,
                                                                                                            constants_and_variables_instantiation_map,
                                                                                                            relations_instantiation_map,
                                                                                                            bound_variables))
    ...
    def instantiate(self, instantiation_map: InstantiationMap) -> \
        ...
        for item in constants_and_variables_instantiation_map:
            if item not in self.templates:
                return None
        for item in relations_instantiation_map:
            if item not in self.templates:
                return None
        try:
...
class Proof:
    ...
    class AssumptionLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            if line.assumption not in assumptions:
                return False
            return line.formula == line.assumption.instantiate(line.instantiation_map)
    ...
    class MPLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            if self.antecedent_line_number >= line_number or self.conditional_line_number >= line_number:
                return False
            line = lines[line_number].formula
            line_ante = lines[self.antecedent_line_number].formula
            line_conclusion = lines[self.conditional_line_number].formula
            try:
                return line == line_conclusion.second and line_conclusion.first == line_ante
    ...
    class UGLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            if self.predicate_line_number >= line_number:
                return False
            f = lines[self.predicate_line_number].formula
            our_formula = lines[line_number].formula
            if not our_formula.root == "A":
                return False
            return our_formula.predicate == f
    ...
    class TautologyLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            assert line_number < len(lines) and lines[line_number] is self
            # Task 9.9
            f = lines[line_number].formula
            prop_formula, m = f.propositional_skeleton()
            return is_propositional_tautology(prop_formula)
...
def axiom_specialization_map_to_schema_instantiation_map(
    ...
    for key,val in propositional_specialization_map.items():
        m[key.upper()]=Formula.from_propositional_skeleton(val, substitution_map)
    return m
...
def prove_from_skeleton_proof(formula: Formula,
    ...
    for line in skeleton_proof.lines:
        if not line.rule == MP:
            propositional_specialization_map = line.rule.specialization_map(PropositionalInferenceRule((),line.formula))
            m = axiom_specialization_map_to_schema_instantiation_map( propositional_specialization_map, substitution_map)
            f = Formula.from_propositional_skeleton(line.formula, substitution_map)
            nline = Proof.AssumptionLine(f,PROPOSITIONAL_AXIOM_TO_SCHEMA[line.rule],m)
            lines.append(nline)
        else:
            f = Formula.from_propositional_skeleton(line.formula, substitution_map)
            nline = Proof.MPLine(f,line.assumptions[0],line.assumptions[1])
            lines.append(nline)
